...
For example suppose we wish to evaluate the second order Taylor
expansion of $f(x)=\sin(x+y)$ for $x=1.1$, about $x=1$.  We could
start with a fifth-order expansion of $\sin(x+y)$:

<<secondordertaylor>>=
(sinxpy <- horner("x+y",c(0,1,0,-1/6,0,+1/120)))
@ 

we would then define an object {\tt dx} with the intent that this is
``small"
...
above we see an {\tt mvp} object equal to
...
numerical value of {\tt x} and {\tt dx}:

<<>>=
(t2 %<>% subs(x=1,dx=0.1))
@ 

For numerical verification we will evaluate this at $x=1, y=0.3$ ,
using $dx=0.1$:
...
$\sin(1.5)=\sin(1.1+0.3+0.1)$ using a Taylor expansion
$\sin(1.4+\delta x)$ to second order in $\delta x$ is ${\mathcal
...
package~\cite{hankin2022_disordR}.  It is possible to examine the
...
probability mass function of its position.  Consider a 4D knight that
moves randomly.  What is its expected Euclidean distance from the
origin after four moves?
...
<<varianceq>>=
onevarpow(K,a=4,b=5)
aderiv(onevarpow(K,a=4,b=5),c=1)
deriv(onevarpow(K,a=4,b=5),"c")
@ 
