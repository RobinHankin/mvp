...
\Shorttitle{The \pkg{mvp} package}
...
\Abstract{
  ...
  on CRAN at \url{https://CRAN.R-project.org/package=mvp}.  To cite
  this article or the package, use~\citet{hankin2022_mvp_tragula}.
}
...
\Repository{https://github.com/RobinHankin/mvp} %% this line for Tragula
...
manipulation of multivariate polynomials in the \proglang{R} programming
language~\citep{rcore2026_tragula}.  It uses the Standard Template
...
retaining the symbolic capabilities of the \pkg{mpoly}
...
over \pkg{mpoly} and offers symbolic features such as substitution,
differentiation, and Taylor expansions.
...
term is a product of powers of algebraic indeterminates (for example,
$x^2y^3z$; note the absence of a coefficient).  We may consider this
term to be the map
...
that powers are nonzero.  An \code{mvp} object is a map from terms to
...
and we understand that coefficients are nonzero.  In \proglang{C++} the
...
representation in the following \proglang{R} session:
...
Coercion and printing are accomplished by the \pkg{mpoly} package
(there is no way I could improve upon
Kahle's work).  Note carefully that the printed representation of
the \code{mvp} object is created by the \pkg{mpoly} package and the print method
...
mathematical properties of the polynomial itself.  In the \pkg{mvp}
...
propitious ordering.  In any event, the \pkg{mpoly} package can
...
Package idiom (\pkg{disordR} discipline) for dealing with the
undefined order of terms is discussed below in
section~\ref{disordsection}.

\subsection{Arithmetic operations}

The arithmetic operations \code{*}, \code{+}, \code{-} and \verb|^| work
as expected:
...
substitute one or more variables for a numeric value.  Define an
\code{mvp} object:
...
Note the natural \proglang{R} idiom, and that the return value is another
\code{mvp} object.  We may substitute for the other variables:
...
the \code{drop} argument:

<<uselose>>=
subs(S3, x = 1, y = 2, z = 3,drop=FALSE)
@ 

The idiom also allows one to substitute a variable for an \code{mvp}
object:
...
Note carefully that \code{subs()} depends on the order of substitution:
...
Function \code{subvec()} allows one to substitute variables for numeric
...
Differentiation is implemented.  First we have the \code{deriv()}
method:
...
Also a slightly different form: \code{aderiv()}, here used to evaluate
...
series.  The \code{trunc()} function allows one to truncate a
...
particular value, we need to use \code{onevarpow()}:
...
We define an \code{mvp} object which is correct to fifth order:

<<secondordertaylor>>=
(sinxpy <- horner("x+y",c(0,1,0,-1/6,0,+1/120)))  # sin(x+y)
@ 

and then define an object \code{dx} with the intent that this is
``small"
...
second order using \code{aderiv()}:
...
above we see an \code{mvp} object equal to the right hand side of
...
(the second argument of function \code{aderiv()} gives the order of
...
numerical value of \code{x} and \code{dx} which we will take to be 0.1:
...
Function \code{series()} will decompose an \code{mvp} object into a power
...
This works nicely with \code{subs()} if we wish to take a power series
about \code{x-v}, where \code{v} is any \code{mvp} object.  For example:

<<powerseriesaboutxminusv>>=
p %>% subs(x="xmv+a+b") %>% series("xmv") 
@ 

is a series in powers of \code{x-a-b}.  We may perform a consistency
...
If function \code{series()} is given a variable name ending in
\code{\_m\_foo}, where \code{foo} is any variable name, then this is
typeset as \code{(x-foo)}.  For example:
...
want to see the expansion of an \code{mvp} object in terms of a more
complicated expression then it is better to use a nonce variable \code{v}:

<<noncevariablev>>=
as.mvp("x^2 + x*a+b^3") %>% subs(x="x_m_v + a^2+b") %>% series("x_m_v")
@ 

where it is understood that $v=a+b^2$.  Function \code{taylor()} is a
...
terms.  Because the terms of an \code{mvp} object have an
implementation-dependent order, this can be difficult.  But we can use
function \code{onevarpow()}:
...
Above we see function \code{onevarpow()} returns an \code{mvp} object
...
The \code{mvp} package handles negative powers, although the idiom is
not perfect and is still under development.  Consider experimental
function \code{invert()}:
...
In the above, \code{p} is a regular multivariate polynomial;
\code{invert(p)} includes negative powers.  It obeys the same
arithmetic rules as other \code{mvp} objects:
...
package~\citep{hankin2022_disordR_tragula}.  It is possible to examine
the coefficients of an \code{mvp} object:
...
Above, note that the result of \code{coeffs()} is a \code{disord}
object, defined in the \pkg{disordR}
package~\citep{hankin2022_disordR_tragula}.  The order of the elements is
...
another \code{mvp} object, \code{b}:
...
Above, we get an error because the coefficients of \code{a} and \code{b}
...
a single \code{mvp} object at will:
...
Extraction also works but subject to standard \pkg{disordR} idiom
restrictions:
...
Extraction methods work, again subject to \pkg{disordR} restrictions:
...
\pkg{magrittr} would simplify the idiom and reduce the opportunity for
error:
...
chess knight is given by function \code{knight()}, whose argument is
...
unrestricted board.  The powers of \code{a} and \code{b} correspond to
