...
List mvp_taylor_onevar(
    ...
    return retval(taylor_onevar(x, (string) v[0], n[0]));
}
...
List mvp_taylor_onepower_onevar(
    ...
    const mvp x = prepare(allnames, allpowers, coefficients);
    return retval(taylor_onepower_onevar(x, (string) v[0], n[0]));
}
...
List mvp_taylor_allvars(
    ...
    const mvp x = prepare(allnames, allpowers, coefficients);
    return retval(taylor_allvars(x, n[0]));
}
...
List mvp_prod(
    ...
    const mvp x1 = prepare(allnames1, allpowers1, coefficients1);
    const mvp x2 = prepare(allnames2, allpowers2, coefficients2);
    return retval(product(x1, x2));  // the meat
}
...
List mvp_add(
    ...
    const mvp x1 = prepare(allnames1, allpowers1, coefficients1);
    const mvp x2 = prepare(allnames2, allpowers2, coefficients2);
    return retval(sum(x1, x2));  // the meat
}
...
List mvp_power(
    ...
    const mvp x = prepare(allnames, allpowers, coefficients);
    return retval(power_int(x,  n[0]));  // the meat
}
