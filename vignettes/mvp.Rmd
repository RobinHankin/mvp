...
If function `series()` is given a variable name ending in `_mfoo`,
where `foo` is any variable name, then this is typeset as `(x-foo)`.
For example:

```{r}
as.mvp('x^3 + x*a') %>% subs(x="x_ma + a") %>% series("x_ma")
```
...
```{r}
as.mvp('x^2 + x*a+b^3') %>% subs(x="x_mv + a^2+b") %>% series("x_mv")
```

where it is understood that $v=a+b^2$.
